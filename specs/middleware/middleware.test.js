...
describe("dataService middleware", () => {
  ...
  describe("fetchCurrencyPageData action", () => {
    ...
    it("should call the data service with url '/page'", () => {
      dataService(testAction).then(() => {
        expect(fakeDataFetch).toHaveBeenCalledWith(
          expect.stringContaining(`/page/${testTarget}`)
        );
      });
    });
    ...
    it("should call the data service with the url '/history'", () => {
      dataService(testAction).then(() => {
        expect(fakeDataFetch).toHaveBeenCalledWith(
          expect.stringContaining(`/history/${testTarget}`)
        );
      });
    });
  });
